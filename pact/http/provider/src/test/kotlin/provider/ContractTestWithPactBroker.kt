...
// Example of provider-side contract test using the pact broker.
// Verification results will be uploaded every time the test is run,
// based on the value of the `pact.verifier.publishResults` property
...
@SpringBootTest(classes = [ContractTestWithPactBrokerConfiguration::class], webEnvironment = RANDOM_PORT)
@ExtendWith(PactVerificationInvocationContextProvider::class)
@DisabledIfEnvironmentVariable(named = "CI", matches = "true") // no broker setup in github actions
internal class ContractTestWithPactBroker(
...
private class ContractTestWithPactBrokerConfiguration {
    @Bean
